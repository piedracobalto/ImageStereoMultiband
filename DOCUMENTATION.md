...
├── ImageStereoMultiband_Installer.exe   ← Compiled installer (Windows)
├── ImageStereoMultiband.dmg             ← Disk image installer (macOS)
│
├── Source/                              ← SOURCE CODE
│   │
...
timerCallback()
  ...
  │     │   └── repaint()
  │     └── Failure: no new data
  │         └── signalHoldCounter++ (scopes not cleared to avoid flicker)
  │
  ├── vectorscope.tickSmoothing()
  │     └── IIR smoothing: displayCorrelation += (target - current) × 0.12
  │
  └── updateBandVisibility()
        └── Recalculates band strip visibility based on +/− buttons
        └── Propagates anySolo to SpectrumCrossoverControls for fill opacity
```
...
**Width control:** Exposed through `Band::setWidth()` with UI parameter 0–100:
- `sideGain = 0.0` (width=0) → Mono (Mid only)
- `sideGain = 1.0` (width=50) → Original stereo width
- `sideGain = 2.0` (width=100) → Widened stereo (double Side component)

The conversion in `Band::setWidth()` is `midSide.setSideGain(width * 0.02f)`.
...
- `setWidth(float)` → `midSide.setSideGain(width * 0.02f)` (converts UI 0–100 to internal 0.0–2.0)
...
│    Band N        │  ← Title
│                  │
│  ──── ○ ────     │  ← Width (horizontal slider, 0–100)
│    Width         │      Value on hover above the slider
│                  │
│       │          │
│       ○          │  ← Gain (vertical slider)
│       │          │     Range: -24 to +24 dB, editable (double-click)
│                  │
│   [ M ] [ S ]    │  ← Mute (M) and Solo (S) buttons
└──────────────────┘
                     ← [−] [+] Band add/remove buttons
```

**How to use:**

1. **Width:** Drag horizontally to adjust the stereo width of the band. Range 0–100: `0` = mid (mono), `50` = original width, `100` = maximum side. The current value is shown on hover over the slider. Double-click to edit numerically.

2. **Gain:** Drag vertically to increase or decrease the band's volume. The dB value is displayed below the slider and is editable with double-click.

3. **M (Mute):** Click to mute the band. The button lights up with the band's color. Muted bands are visually dimmed to 5% opacity.

4. **S (Solo):** Click to isolate the band. If you activate Solo on any band, all others are automatically muted. You can activate multiple solos simultaneously. Non-soloed bands (when another solo is active) are visually dimmed to 5% opacity.

5. **Add/Remove:** The **−** (remove) and **+** (add) buttons at the bottom of the band panel allow hiding/showing bands. The outermost band is removed on **−** click; a new band is added on **+** click.
...
- **Horizontal grid lines:** -60, -40, -20 dB (-80 removed to avoid visual clutter; right gap added)
- **Vertical frequency marks:** 30, 60, 100, 300, 1k, 5k, 15k Hz
- **Crossover labels:** format "120", "2.0k", "10.0k" depending on magnitude. Vertically staggered (+0, +8, +16, +24px) to prevent overlap. Double-click the value to edit with keyboard
- **"Hz" label** at the bottom-right corner of the graph
- **Bottom margin:** 48px (increased from 36px to accommodate staggered labels)
...
| **W** | White mode indicator (moved 6px left from previous version) |

#### Anti-flicker

The vectorscope no longer clears scopes on every timer tick when no new data is available. Instead, a `signalHoldCounter` retains the image for 5 frames before fading, eliminating flicker during silences.
...
|-----------|----|------|-------|---------|
| Width band N | `band{N}Width` | Float (integers) | 0 – 100 | 50 |
...
**Purpose:** Generates the plugin's 25 parameters. Iterates 5 bands creating Width (float 0–100 with step 1, default 50), Gain (float -24–24 dB), Mute (bool), Solo (bool). Then creates 4 crossovers (float 20–20000 Hz) with default values (120, 500, 2000, 8000). Finally creates the bypass (bool).
